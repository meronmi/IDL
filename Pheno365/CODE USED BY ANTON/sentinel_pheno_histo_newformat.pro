FUNCTION Sentinel_pheno_histo_newFormat, dir
  vars =   ['SOS50', 'EOS50','LGS50','SOS20','p6','PS90']
...
;  ENDFOR
  ...
  subWinPos = [1,2,3,5,6,7]
  hH = OBJARR(N_ELEMENTS(vars))
  ;make histograms
  ...
  FOR k = 0, N_ELEMENTS(indices)-1 DO BEGIN
    ...
    FOR u = 0, 1 DO BEGIN ;all data, only selected based on gaps and perror  
      FOR i = 0, N_ELEMENTS(bigCropTypes)-1 DO BEGIN 
        ...
        res = Raphael_data_select_newFormat(x, u, indices[k], bigCropTypes[i])
        ...
        FOR j = 0, N_ELEMENTS(vars)-1 DO BEGIN
          ...
          y =  [x.(WHERE(TAG_NAMES(x) EQ 'G1_'+STRUPCASE(vars[j])))[res.indRetr], x.(WHERE(TAG_NAMES(x) EQ 'G2_'+STRUPCASE(vars[j])))[res.indRetr]]
          indFin = WHERE(y NE -9999, nFin)
          y = y[indFin]
          pdf = HISTOGRAM(y, NBINS=20, LOCATIONS=xbin)
          pdf = pdf / FLOAT(TOTAL(pdf))
          nameStr = tmpName + ' n' + STRTRIM(nFin,2) ;+ ' f' + STRTRIM(res.countRetrFailure,2) 
          IF (i EQ 0) THEN BEGIN
            IF (j LE 2) THEN mrg = [0.25,0.05,0.05,0.2] ELSE mrg = [0.25,0.125,0.05,0.14] ; [left, bottom, right, top]
            IF (j EQ 0) THEN hH[j] = PLOT(xbin, pdf, COLOR=ctable[*,i], YTITLE='Frequency', XTITLE=titles[0], NAME=nameStr, DIMENSIONS=[1200,600], LAYOUT=[4,2,subWinPos[j]], MARGIN=mrg) $
            ELSE hH[j] = PLOT(xbin, pdf, COLOR=ctable[*,i], YTITLE='Frequency', XTITLE=titles[0], NAME=nameStr, DIMENSIONS=[1200,600], LAYOUT=[4,2,subWinPos[j]], MARGIN=mrg, /CURRENT)
          ENDIF ELSE BEGIN
            hH[j].Select
            hH[j] = PLOT(xbin, pdf, COLOR=ctable[*,i], YTITLE='Frequency', NAME=nameStr, /OVERPLOT)
          ENDELSE
          IF (j EQ 0) THEN h1_array = [h1_array,  hH[j]]
        ENDFOR 
;        ;y = x.eos20[res.indRetr]
;        y =  [x.(WHERE(TAG_NAMES(x) EQ 'G1_'+STRUPCASE(vars[1])))[res.indRetr], x.(WHERE(TAG_NAMES(x) EQ 'G2_'+STRUPCASE(vars[1])))[res.indRetr]]
;        indFin = WHERE(y NE -9999, nFin)
;        y = y[indFin]
...
;        y =  [x.(WHERE(TAG_NAMES(x) EQ 'G1_'+STRUPCASE(vars[2])))[res.indRetr], x.(WHERE(TAG_NAMES(x) EQ 'G2_'+STRUPCASE(vars[2])))[res.indRetr]]
;        indFin = WHERE(y NE -9999, nFin)
;        y = y[indFin]
...
;        y =  [x.(WHERE(TAG_NAMES(x) EQ 'G1_'+STRUPCASE(vars[3])))[res.indRetr], x.(WHERE(TAG_NAMES(x) EQ 'G2_'+STRUPCASE(vars[3])))[res.indRetr]]
;        indFin = WHERE(y NE -9999, nFin)
;        y = y[indFin]
...
;      
;        ;y = x.p6[res.indRetr]
;        y =  [x.(WHERE(TAG_NAMES(x) EQ 'G1_'+STRUPCASE(vars[4])))[res.indRetr], x.(WHERE(TAG_NAMES(x) EQ 'G2_'+STRUPCASE(vars[4])))[res.indRetr]]
;        indFin = WHERE(y NE -9999, nFin)
;        y = y[indFin]
...
;        y =  [x.(WHERE(TAG_NAMES(x) EQ 'G1_'+STRUPCASE(vars[5])))[res.indRetr], x.(WHERE(TAG_NAMES(x) EQ 'G2_'+STRUPCASE(vars[5])))[res.indRetr]]
;        indFin = WHERE(y NE -9999, nFin)
;        y = y[indFin]
...
;        ENDELSE
      ...
      hH[0].save, dir_out + '\Histo_pheno_params_' + prefix + '_' + res.suffix + '.png'
      hH[0].close
    ENDFOR; k
  ENDFOR; u
  RETURN, 0
