PRO Cozmin_read_once_and_sav_v3
  ;now treat only s2
  
  csv_file = 'D:\COZMIN_pheno_test\Thuin_S2.csv';D:\RAPHAEL_pheno_test\LUCAS_S1-S2_extraction_20190520.csv'
  
  res = READ_CSV(csv_file, HEADER=hdr)
  x = rename_tags(res, TAG_NAMES(res), ['pid','pointID','prID','type','campagne','crop_ID','cropname','date','index_value','tule','index_name','x','y'])
   
  ;make a big table
  ;indS1 = WHERE(x.index_name EQ 'VH_mean', ns1)
  ns1 = 0
  indS2 = WHERE(x.index_name EQ 'NDVI_ME', ns2)
  ntot = ns1 + ns2
...
;  ind = 0
  ...
  ;now S2
  ind = WHERE(x.index_name EQ 'NDVI_ST', n)
...
;    surv_date[i] = '20' + STRTRIM(tmp[2],2) + '-' + STRING(MM, FORMAT='(I02)') + '-' + STRTRIM(tmp[0],2)
;  ENDFOR
  y.surveyDateJD[ns1:-1] = -9999;YYYYbMMbDD2jd(surv_date)
  y.s12[ns1:-1] = 2
  fn = FILE_DIRNAME(csv_file) + '\' +FILE_BASENAME(csv_file, '.csv') + '_v0.sav';STRSPLIT(csv_file, '.', /EXTRACT)
  SAVE, y,  FILENAME=fn
END