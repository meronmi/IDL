FUNCTION Raphael_data_select_newFormat, x, u, index, cropType
...
CASE u OF
  0: BEGIN
    ind = WHERE((x.index EQ index) AND (x.crop_type EQ cropType), count)
    subTitle = 'All retrieval';, n=' +STRTRIM(N_ELEMENTS(ind),2)
    suffix = 'all_retrieval'
  END
  1: BEGIN
    ind = WHERE((x.index EQ index) AND (x.crop_type EQ cropType) AND $
      (x.G1_PRCTHETEROGOBS_INSEASON LE 20) AND (x.G2_PRCTHETEROGOBS_INSEASON LE 20), count, /NULL)
    ;now get their ID
;    ids = x.ID[ind]
;    ind2Keep = !NULL
;    ;check which ID has NDVI prctHeterogObs_inSeason less than 20 and keep those only   
;    FOR i = 0, count-1 DO BEGIN
;      indNDVI = WHERE((x.ID EQ ids[i]) AND (x.Index EQ 'NDVI'))
...
;  END
ENDCASE
ret = CREATE_STRUCT('indRetr', ind, 'countRetr',N_ELEMENTS(ind), 'subTitle', subTitle, 'suffix', suffix)
