FUNCTION findMinAwayFromMax_v4, subExtrema, sig, subRegMax, x, y, direction, holeMinFractionalImportance, yobs
...
FOR i = 0, countMin-1 DO BEGIN
  ...
  IF (min_found EQ 0) THEN BEGIN
    ...
    den = TOTAL(FINITE(yobs[subExtremaSegment[subOfMin[i]-1]:subExtremaSegment[subOfMin[i]+1]]))-1
    varOfFit = 1.0/den $
      * TOTAL((y[subExtremaSegment[subOfMin[i]-1]:subExtremaSegment[subOfMin[i]+1]] - yobs[subExtremaSegment[subOfMin[i]-1]:subExtremaSegment[subOfMin[i]+1]])^2,/NAN)
    ;test statistics = (y_min - MAX(max0, max1))/varOfFit
    stat = (ySegmnet[subOfMin[i]]-MAX([ySegmnet[subOfMin[i]-1], ySegmnet[subOfMin[i]+1]]))/sqrt(varOfFit)
