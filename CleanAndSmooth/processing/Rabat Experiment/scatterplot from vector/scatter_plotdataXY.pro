...
pro scatter_plotDataXY, data1, data2, x_label, y_label, dir_output, data1_range, data2_range
...
;xmargin=[1,1]               
;ymargin=[1,1]
ngrid=100 ;200                 ;resolution of the plot
xsize=600 & ysize=600    ;dimensions of the window
charsize=1.;1.
charthick=1.4
tit='Scatter-plot ' + x_label + ' vs. ' + y_label 
xtit=x_label
ytit=y_label
xthick=1.5
ythick=1.5
posleg=[0.7,0.15]       ;legend position
write_stat=['OLSajuste','GMRFajuste', 'RMS','r', 'AC','dfAC']
;********** HISTOGRAM PARAMETERS
frec_min= 0                ; only represents the values with a frequency > frec_min
frec_max= 200000;20000               ; saturate values with frequency > frec_max (if no defined is calculated authomatically)
;frec_max= 200000
...
makechar,res[1],3,Gslopechar
makechar,res[0],3,Gconstchar
...
for i=0,nstat-1 do begin
   ...
   xyouts,inix,iniy-0.1*(i),tit(i),color=ccolor, charsize=1.0,charthick=1.2
