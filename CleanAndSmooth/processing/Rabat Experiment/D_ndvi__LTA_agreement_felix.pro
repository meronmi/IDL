PRO D_ndvi__LTA_agreement_felix
...
;s1 = 'ot'
;s1_dir = base_dir + '\METOP\HIS\HIS_on_s'
;s2 = 'ea'
;s2_dir = base_dir + '\EMDODIS\HIS\resample1km'
s1 = 'ea'
s1_dir = base_dir + '\EMDODIS\HIS\resample1km'
s2 = 'pv'
s2_dir = base_dir + '\VGT-PV\HIS_on_s\METOP_WIN'
...
;  scatter_plotDataXY, s1_float[indFinBoth], s2_float[indFinBoth], 'nd_dek_'+deks[i]+'_'+s1, s2, base_dir, [-1.0,1.0], [-1.0,1.0] 
...
hh.save, base_dir + '\LTA_NDtimeAC_' + s1 + 'VS' + s2 + '.png'
...
scatter_plotDataXY, x, y, 'LTA_nd_'+s1, 'LTA_nd_'+s2, base_dir, [0.0,1.0], [0.0,1.0] 
...
stats2[1:*,*] = stats
stats2[0,*] = deks
WRITE_CSV, base_dir + '\' + 'LTA_ND_' + s1 + 'VS' + s2 + '_agreement_stats.csv', stats2, HEADER=strStats
PRINT, 'Done.'
END