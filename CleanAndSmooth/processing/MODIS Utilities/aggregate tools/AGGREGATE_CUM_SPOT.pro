...
PRO AGGREGATE_CUM_SPOT

STARTTIME = SYSTIME(1)

; General settings (to be changed for different input)
dataPath='G:\BACKUP\IMAGES\IBLI\SPOT\'         ; location where administrative region-file is located
which = ['cumNDVI','maxNDVI']                   ; select which to aggregate
which = which[0]
imageFileBase = 'SPOT_VGT_filtered_Kenya.img'  ; original filtered NDVI stack file
...
nSeasons=27                        ; number of seasons in cumulated NDVI file
firstYear=1998                     ; first year in the cumulated dataset
...
IF which eq 'cumNDVI' THEN lineAssCumNDVI = ASSOC(R1, INTARR(ns,nSeasons))    ; Associate a line of data (with all bands)
IF which eq 'maxNDVI' THEN lineAssCumNDVI = ASSOC(R1, BYTARR(ns,nSeasons)) 
...
outFile = workPath+which+'_aggregated'+'_SPOT'+ '.csv'
