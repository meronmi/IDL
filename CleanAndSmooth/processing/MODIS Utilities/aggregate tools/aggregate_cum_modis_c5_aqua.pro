...
PRO AGGREGATE_CUM_MODIS_C5_Aqua

STARTTIME = SYSTIME(1)

; General settings (to be changed for different input)
dataPath='K:\ILRI\MODIS_C5_aqua\bil_files\'         ; location where administrative region-file is located
which = ['cumNDVI','maxNDVI']                   ; select which to aggregate
which = which[1]
imageFileBase = 'MODIS_C5_Aqua_NDVI_bil_cl_Sm_Ch'  ; original filtered NDVI stack file
...
;adminFile = 'K:\ILRI\ILRI shp files\IBLI_108_div_rasters\IBLI108div_on_MODIS_c5_Terra.img'
adminFile = 'K:\ILRI\ILRI shp files\IBLI_84_div_rasters\divisionID_NASA.img'

nl=4658                            ; number of lines
ns=3808                            ; number of samples
nSeasons=20                        ; number of seasons in cumulated NDVI file
firstYear=2002                     ; first year in the cumulated dataset
...
outFile = workPath+which+'_aggregated'+'_MODIS_C5_AQUA'+ '.csv'
