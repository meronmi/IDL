...
;;;;;
  ...
  ;Michele + 29 Oct 2015, give NaN back
  ...
  bigA = alog(posave) - (logsum/pvals)  ;A = ln(x_avg) - SUM(x)/n
  shape = !VALUES.F_NAN		;   added by greg 2012.04.19
  scale = !VALUES.F_NAN		;   added by greg 2012.04.19
  if bigA gt 0 then begin
     shape = (1.0+sqrt((4.0*bigA/3.0)+1.0)) / (4.0*bigA)  ;alpha
     scale = posave / shape   ;beta
  end
