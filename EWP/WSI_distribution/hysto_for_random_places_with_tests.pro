FUNCTION hysto_for_random_places_with_tests, ns, kernel_size
  ...
  FOR f = 0, N_ELEMENTS(prog) - 1 DO BEGIN
    ...
    FOR j = 0, ns-1 DO BEGIN
      ...
      yk[*,f,j] = REFORM(mat[s-hks:s+hks,l-hks:l+hks,*], bands*ks*ks )
    ENDFOR
  ENDFOR
  FOR j = 0, ns-1 DO BEGIN
    res = ARRAY_INDICES(mmap,rndind[j])
    s = res[0]
    l = res[1]
    ;make the histogram for individual pixels
    pdf = LONARR(n, N_ELEMENTS(prog))
    FOR i = 0, N_ELEMENTS(prog) - 1 DO BEGIN
      ;extract data remove Nan coming from places that have 24 observations
      tmp = y[*,i,j]
      indf = WHERE(FINITE(tmp))
      yATprog = tmp[indf]
      pdf[*,i] =  HISTOGRAM(yATprog, MIN=0.0, MAX=100.0, NBINS=n,  LOCATIONS=xbin)
    ...
    ;make the histogram for kernels
    pdfk = FLTARR(nk, N_ELEMENTS(prog))
    FOR i = 0, N_ELEMENTS(prog) - 1 DO BEGIN
      tmp = yk[*,i,j]
      indf = WHERE(FINITE(tmp))
      ykATprog = tmp[indf]
      ;fit a beta and a normal, test, compute likelihood
      ;ret = FIT_BETA(ykATprog/100.0)
      ;x = FINDGEN(101, INCREMENT=0.01, STAR=0.0)
      ;BetaFitPdf = IMSL_BETACDF(x[1:100], ret[0], ret[1]) - IMSL_BETACDF(x[0:99], ret[0], ret[1])
      ;plot the histogram
      pdfk[*,i] =  HISTOGRAM(ykATprog, MIN=0.0, MAX=100.0, NBINS=nk,  LOCATIONS=xbink)
      ;normalize it 
      ;pdfk[*,i] = pdfk[*,i] / DOUBLE(TOTAL(pdfk[*,i], /DOUBLE))
      IF (i EQ 0) THEN BEGIN
       ymaxk = MAX(pdfk) + 2
