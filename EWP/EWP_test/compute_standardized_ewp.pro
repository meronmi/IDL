PRO compute_standardized_ewp, fn_base_out, fn_ewp_in, half_life
...
;      IF (s eq 632) AND (l eq 115) THEN BEGIN
;        PRINT, 'debug'
;      ENDIF
      FOR TT = 1, 36 DO BEGIN
        ;find all the images of a given dekad
        indTT = WHERE(REFORM(timeArray[1, *]) EQ TT)
        tmp = precip_2_spi_gh(ewp[s,indTT], MIN_POSOBS = 3)
