PRO test_cop_handler
truncation2rangeTeor = [0];[0,1]
variables = ['z'];['z','n','v']
version = '1' ;version of anonaly to be used
...
FOR i = 0, N_ELEMENTS(variables)-1 Do BEGIN
  FOR j = 0, N_ELEMENTS(truncation2rangeTeor)-1 Do BEGIN
    test_cop_O_stages, variables[i], truncation2rangeTeor[j], version
  ENDFOR
ENDFOR
END

PRO test_cop_O_stages, variable, trunc2rangeTeor, version
TIC  
...
ENDIF ELSE BEGIN
  screen = 'Win'
  dirsep = '\'
  main_dir = '\\ies\d5\foodsec\Share\COP_FAPAR'
ENDELSE
plot_only_act_veg = 1  
strPeriod = '1634-1831' ; + '_' + strPeriod + '_' +
...
dir = main_dir + dirsep + 'thinned' + dirsep + STRUPCASE(variable) + version
...
tmp = ReadEnviWithHdr(main_dir+dirsep+'thinned'+dirsep+'pheno'+dirsep+'LTAR6_KK0.img')
...
varNfLf = ReadEnviWithHdr(dir + dirsep + variable + 'NfLf_'+ strPeriod +'_bsq.img')
;set inf to Nan
...
sz = SIZE(varNfLf)
;apply the finite mask to remove 18 pixels where OF time series has missing values [COP:should not be the case]
...
IF (target NE 'all') THEN BEGIN
  ...
  CASE target OF ;NOT NEEDED FOR COP, I use all
    'cro': fn = main_dir + dirsep + 'thinned' + dirsep + 'masks' + dirsep + 'mask_crop_afi_asap2.img'
    'range': fn = main_dir + dirsep + 'thinned' + dirsep + 'masks' + dirsep + 'mask_pasture_afi_asap2.img'
    ELSE: stop
  ENDCASE
...
FOR i = 1, 36 DO BEGIN
  ...
  active[*,*,i-1] = FLOAT(ReadEnviWithHdr(main_dir+dirsep+'thinned'+dirsep+'pheno'+dirsep+'active'+dirsep+'active1962'+tt+'.img'))
...
;BE CARE WITH COP, the period is 1634 to 1831, 70 dekads (bands)
active =  [[[active[*,*,33:35]]], $ ;year 2016
          [[active]], $ ;year 2017
          [[active[*,*,0:30]]]] ;year 2018
...
;COP:  row will not be used 
mnAbsDiffVarNxLxf_varNfLf = FLTARR(3,2,3) ;row: 0 = NxLf, 1 = NxLx. Dimension 3 is for: all pixels/vegetated/vegetated and active
;var for mean difference
mnDiffVarNxLxf_varNfLf = FLTARR(3,2,3) ;row: 0 = NxLf, 1 = NxLx. Dimension 3 is for: all pixels/vegetated/vegetated and active
...
prctMissInNiLjWrNfLf = FLTARR(3,2,3)
;of an anomoly made with NxLy, the percent of pixels missing (they are NaN) in NfLf. It is the % of all valid NxLy pixels
prctMissInNfLfWrNiLj = FLTARR(3,2,3)

;files to read
;files are [N0Lf, .., N4Lf]
;          [N0L0, .., N4L4]
cop_stages = [0,1,2]
fn_varNxLxf = [[dir + dirsep + variable + 'N' + STRTRIM(cop_stages,2) + 'Lf_'+strPeriod+'_bsq.img']];,$
              ;[dir + dirsep+ variable + 'N' + STRTRIM(INDGEN(5),2) + 'L'+ STRTRIM(INDGEN(5),2) + '_'+strPeriod+'_bsq.img']]     ;5 colums, 2 rows
NL_names =['N' + STRTRIM(cop_stages,2) + 'Hf'];,['N' + STRTRIM(INDGEN(5),2) + 'H'+ STRTRIM(INDGEN(5),2)]]
...
veg_act_stats = CREATE_STRUCT('mc_a',FLTARR(3,2),'mc_HSS',FLTARR(3,2),'mc_rank_a',FLTARR(3,2),'mc_rank_mm',FLTARR(3,2),'mc_rank_m',FLTARR(3,2),'mc_rank_sm',FLTARR(3,2),'mc_rank_um',FLTARR(3,2),$
                              'd_a',FLTARR(3,2),'d_bias',FLTARR(3,2), 'd_DetRate', FLTARR(3,2), 'd_FalseAlRate', FLTARR(3,2), 'd_HSS', FLTARR(3,2)) ;row: 0 = NxLf, 1 = NxLx
FOR jLTA = 0, 0 DO BEGIN ;loop on consolidation stagle of LTA: either f (jLTA = 0) 
  FOR iNRT = 0, 2 DO BEGIN ; loop on consolidation stage of NRT, 3 in total, 0 to 2
...
;    ENDIF
    ...
    ;sam but scale -5 to 5 (z)
    IF (variable EQ 'z') THEN BEGIN
      DensityAndFit_log_scale, REFORM(varNiLj*active, N_ELEMENTS(varNiLj*active)), REFORM(varNfLf*active, N_ELEMENTS(varNfLf*active)), variable+NL_names[iNRT,jLTA], variable+'NfHf', dir_results, $
        [-5,5], [-5,5],  $;data1_range, data2_range,
        nbins, 30, $;TITLE='Act.veg', $;ngrid, nlevels, TITLE=title, $
        DOFIT=1, SIMPLE=1, DOLOG=1, PLOT1TO1=1, FILESUFFIX = '_act_veg_nbins_RANGE_REDUCED' + STRTRIM(nbins,2), NOWIN=1
    ENDIF
;    DensityAndFit_log_scale, REFORM(varNiLj*active, N_ELEMENTS(varNiLj*active)),  REFORM(varNfLf*active, N_ELEMENTS(varNfLf*active)), variable+NL_names[iNRT,jLTA], variable+'NfHf', dir_results, $
...
gh1vegact = PLOT(mnAbsDiffVarNxLxf_varNfLf[*,0,2], SYMBOL= 'o', NAME = variable + 'NxHf', $ ; LINESTYLE = ':', COLOR= 'green',
                 XTITLE='Consolidation Stage x', YTITLE = 'MAE ('+ variable + 'NxHy - '+ variable + 'NfHf)', XRANGE=xrg, FONT_SIZE = fs_axis, MARGIN = marg, XTICKVALUES=[0,1,2], XMINOR = 0)
...
;gh1vegact = PLOT(mnDiffVarNxLxf_varNfLf[*,0,2], SYMBOL= 'o', NAME = variable + 'NxHf_actVeg', $ ;, LINESTYLE = ':', COLOR= 'green'
  gh1vegact = PLOT(mnDiffVarNxLxf_varNfLf[*,0,2], SYMBOL= 'o', NAME = variable + 'NxHf', $ ;, LINESTYLE = ':', COLOR= 'green'
                 XTITLE='Consolidation Stage x', YTITLE = 'ME ('+ variable + 'NxHy - '+ variable + 'NfHf)', XRANGE=xrg, FONT_SIZE = fs_axis, MARGIN =marg, XTICKVALUES=[0,1,2], XMINOR = 0)
targetList = [targetList, gh1vegact]
...
gh1 = PLOT(prctMissInNiLjWrNfLf [*,0,0], SYMBOL= 'o', LINESTYLE = ':', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', YTITLE = '% missing in NxHy but present in NfHf', XRANGE=xrg, XTICKVALUES=[0,1,2], XMINOR = 0)
...
ghl = LEGEND(TARGET=[gh1,gh1veg,gh1vegact], SHADOW = 0, TRANSPARENCY = 100, POSITION = [0.9,0.85])
...
;gh1 = PLOT(prctMissInNiLjWrNfLf [*,0,0], SYMBOL= 'o', LINESTYLE = ':', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', YTITLE = '% missing in NxHy but present in NfHf', XRANGE=xrg)
;;gh2 = PLOT(prctMissInNiLjWrNfLf [*,1,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHx', /OVERPLOT)
;gh1veg = PLOT(prctMissInNiLjWrNfLf [*,0,1], SYMBOL= 'o', LINESTYLE = ':', COLOR= 'red', NAME =  variable + 'NxHf_veg', /OVERPLOT)
;;gh2veg = PLOT(prctMissInNiLjWrNfLf [*,1,1], SYMBOL= 'o', LINESTYLE = '-', COLOR= 'red', NAME = variable + 'NxHx_veg', /OVERPLOT)
;gh1vegact = PLOT(prctMissInNiLjWrNfLf [*,0,2], SYMBOL= 'o', LINESTYLE = ':', COLOR= 'green', NAME = variable + 'NxHf_actVeg', /OVERPLOT)
...
IF (variable EQ 'z') OR (variable EQ 'v') OR (variable EQ 'n') OR (variable EQ 's')THEN BEGIN
  ...
  gh1 =  PLOT(veg_act_stats.d_DetRate[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
              YTITLE = 'Detection Rate [0,100]', XRANGE=xrg, FONT_SIZE = fs_axis, XTICKVALUES=[0,1,2], XMINOR = 0);, LAYOUT=[3,2,1]
  ;gh1b = PLOT(veg_act_stats.d_DetRate[*,1], SYMBOL= 'o', LINESTYLE = '-', COLOR = 'red', NAME = variable + 'NxHx', /OVERPLOT) 
  ;otitle = TEXT(0.5, 0.99titleString, ALIGNMENT = 0.5, VERTICAL_ALIGNMENT = 1.0)
  ghl = LEGEND(TARGET=[gh1], SHADOW = 0, TRANSPARENCY = 100, POSITION = posleg, ORIENTATION = 1, HORIZONTAL_ALIGNMENT = 'CENTER', VERTICAL_ALIGNMENT = 1, FONT_SIZE = fs_legend)
  ...
  gh2 =  PLOT(veg_act_stats.d_FalseAlRate[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
    YTITLE = 'False Alarm Rate [0,100]', XRANGE=xrg, FONT_SIZE = fs_axis, XTICKVALUES=[0,1,2], XMINOR = 0); , LAYOUT=[3,2,2], /CURRENT
  ...
  ghl = LEGEND(TARGET=[gh2], SHADOW = 0, TRANSPARENCY = 100, POSITION = posleg, ORIENTATION = 1, HORIZONTAL_ALIGNMENT = 'CENTER', VERTICAL_ALIGNMENT = 1, FONT_SIZE = fs_legend)
  ...
  gh1 =  PLOT(veg_act_stats.d_DetRate[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf DR', XTITLE='Consolidation Stage x', $
    YTITLE = 'DR and FAR', XRANGE=xrg, YRANGE=[0,100], FONT_SIZE = fs_axis, XTICKVALUES=[0,1,2], XMINOR = 0);, LAYOUT=[3,2,1]
  ...
  ghl = LEGEND(TARGET=[gh1, gh2], SHADOW = 0, TRANSPARENCY = 100, POSITION = posleg, ORIENTATION = 1, HORIZONTAL_ALIGNMENT = 'CENTER', HORIZONTAL_SPACING=0.03,VERTICAL_ALIGNMENT = 1, FONT_SIZE = fs_legend)
  ...
  gh2 =  PLOT(veg_act_stats.d_bias[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
    YTITLE = 'Bias', XRANGE=xrg, FONT_SIZE = fs_axis, YRANGE=[0,140], XTICKVALUES=[0,1,2], XMINOR = 0) ; , LAYOUT=[3,2,3], /CURRENT
  ...
  gh2 =  PLOT(veg_act_stats.d_HSS[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
    YTITLE = 'HSS', XRANGE=xrg, FONT_SIZE = fs_axis, YRANGE=[0,1], XTICKVALUES=[0,1,2], XMINOR = 0) ; , LAYOUT=[3,2,4], /CURRENT
  ...
  gh2 =  PLOT(veg_act_stats.d_a[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
    YTITLE = 'Overall Accuracy', XRANGE=xrg, FONT_SIZE = fs_axis, YRANGE=[0,100], XTICKVALUES=[0,1,2], XMINOR = 0) ;, LAYOUT=[3,2,5], /CURRENT
  ...
  gh2 =  PLOT(veg_act_stats.mc_a[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
    YTITLE = 'Overall Accuracy', XRANGE=xrg, FONT_SIZE = fs_axis, YRANGE=[0,100], XTICKVALUES=[0,1,2], XMINOR = 0)  ;, LAYOUT=[3,2,6], /CURRENT
  ...
  gh2 =  PLOT(veg_act_stats.mc_HSS[*,0], SYMBOL= 'o', LINESTYLE = '-', NAME = variable + 'NxHf', XTITLE='Consolidation Stage x', $
    YTITLE = 'HSS', XRANGE=xrg, FONT_SIZE = fs_axis, YRANGE=[0,1], XTICKVALUES=[0,1,2], XMINOR = 0)  ;, LAYOUT=[3,2,6], /CURRENT
  ...
  b1 = BARPLOT(a,   FILL_COLOR='#2c7bb6', YRANGE=[0,100], XRANGE=[-0.6,2.6], NAME = 'A', $ ;LAYOUT=[1,2,1],
       MARGIN = marg, $;TITLE = 'Multi-category agreement, ' + variable + 'NxHf',  MARGIN = marg, $
       XTITLE='Consolidation stage x', YTITLE='%',FONT_SIZE = fs_axis, XTICKVALUES=[0,1,2], XMINOR = 0)
  ...
  ghl = LEGEND(TARGET=[b1, b2, b3, b4, b5], SHADOW = 0, TRANSPARENCY = 100, POSITION = [0.5,0.2], ORIENTATION = 1, HORIZONTAL_ALIGNMENT = 'CENTER', VERTICAL_ALIGNMENT = 1, FONT_SIZE = fs_legend)
  ;now HxLx
