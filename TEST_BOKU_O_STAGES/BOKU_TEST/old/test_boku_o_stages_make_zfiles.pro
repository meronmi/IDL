PRO test_boku_O_stages_make_Zfiles
  ...
  PRINT, SYSTIME()
  zNfLf = Z_by_dek_ARRAY3D(xf, 1, 36, MINVAL = 0, MAXVAL = 250, GAIN = 0.0048, OFFSET = -0.2, STATFNAME=dir+'\LTA_STATS\OF_dek_')
  res = write_envi_img(zNfLf, dir + '\Z\zNfLf_' + strPeriod + '_bsq')
  res = write_envi_hdr(dir + '\Z\zNfLf_' + strPeriod + '_bsq.hdr', ns, nl, 4, NBANDS=sz[3], INTERLEAVE='bsq')
  ...
  FOR i = 0, 4 DO BEGIN
    ...
    PRINT, SYSTIME()
    zNxLf = Z_by_dek_ARRAY3D(REFORM(x[*,*,*,i]), 1, 36, XHIS = xf, MINVAL = 0, MAXVAL = 250, GAIN = 0.0048, OFFSET = -0.2)
    PRINT, SYSTIME()
    res = write_envi_img(zNxLf, dir + '\Z\zN' + STRTRIM(i,2) + 'Lf_' + strPeriod + '_bsq')
    res = write_envi_hdr(dir + '\Z\zN' + STRTRIM(i,2) + 'Lf_' + strPeriod + '_bsq.hdr', ns, nl, 4, NBANDS=sz[3], INTERLEAVE='bsq')
...
;  ENDFOR
  ...
  FOR i = 0, 4 DO BEGIN
    PRINT, 'Mem: ' + STRTRIM(memory(/Current)/1000000.0,2)
    PRINT, SYSTIME()
    zNxLx = Z_by_dek_ARRAY3D(REFORM(x[*,*,*,i]), 1, 36, MINVAL = 0, MAXVAL = 250, GAIN = 0.0048, OFFSET = -0.2, STATFNAME=dir+'\LTA_STATS\O'+STRTRIM(i,2)+'_dek_')
    PRINT, SYSTIME()
    res = write_envi_img(zNxLx, dir + '\Z\zN' + STRTRIM(i,2) + 'L' + STRTRIM(i,2) +'_' + strPeriod + '_bsq')
    res = write_envi_hdr(dir + '\Z\zN' + STRTRIM(i,2) + 'L' + STRTRIM(i,2) +'_' + strPeriod + '_bsq.hdr', ns, nl, 4, NBANDS=sz[3], INTERLEAVE='bsq')
    zNxLx = 0
  ENDFOR
