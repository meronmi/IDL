FUNCTION Pheno_indicator_builder_GLOBRAD
@cb_io.comm
...
;
;With Pheno_indicator_builder_GLOBRAD it computes FAPAR*GLOBRAD
...
globrad_product_dir = 'GlobradECMWF'
globrad_product = 'bil_int_ope9810-1217' ;must be a bil file with dekad from 10 1998 to at least the last dekad in the RS data
globrad_fname = base_path + '\' + globrad_product_dir + '\' + globrad_product
nb_gr = 512
...
grAssVar = ASSOC(lungr, DBLARR(ns,nb_gr))
  ...
  FOR d=0, N_ELEMENTS(ftd)-1 DO BEGIN
    FOR b=0, 1 DO BEGIN ;baseline sub or not
      ...
      FOR line=0, nl-1, 1L DO BEGIN   ;loop on lines
        ...
        ;check for 0 values
        ind0 = WHERE(grline EQ 0, count0)
        IF (count0 GT 0) THEN grline[ind0]=!VALUES.F_NAN 
            ...
            IF (FINITE(p0line[sample,y]) EQ 1) THEN BEGIN 
              IF (p0line[sample,y] NE -999) THEN BEGIN 
                ...
                pdhtf_pars=[p0line[sample,y], p1line[sample,y], p2line[sample,y], $
                            ...
                            p6line[sample,y]]
                gry=grpix[t0line[sample,y]:t1line[sample,y]]            ;globrad of that year, from t0 to t1
                ;If (sample EQ 279) AND (line EQ 126) THEN STOP
                pheno_line[sample,y] = compute_acc_GlobRad(pdhtf_pars, t0line[sample,y], t1line[sample,y], resampling, $
                                                   ftg[g], ftd[d], b, gry)
