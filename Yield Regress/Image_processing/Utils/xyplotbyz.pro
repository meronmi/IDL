FUNCTION XYplotByZ, x, y, z, wt, pt, xt, yt, doplot, legend
...
IF (doplot EQ 1) THEN BEGIN
  ...
  ;base plot  
  PLOT, x, y, TITLE=pt, XTITLE=xt, YTITLE=yt, XSTYLE=1, $
        PSYM=1, /NODATA, XRANGE=xr, YRANGE=yr, BACKGROUND = 16777215, COLOR = 0 
  usepsym=[1,2,4,5,6,7]
  p=-1
  DEVICE, DECOMPOSED=0 
  LOADCT, 38, /SILENT
  FOR d=0,  n-1 DO BEGIN
    ...
    IF count GT 0 THEN BEGIN
      ...
      XYOUTS, xp1, yr[1]-((yr[1]-yr[0])/double(2*n))*(d+1), strtrim(FIX(zlist[d]),2)+' '+legend[d], COLOR=clr, CHARSIZE=1.0 
