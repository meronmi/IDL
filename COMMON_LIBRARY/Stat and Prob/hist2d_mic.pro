...
;        23 nov 2017        Avoids considered as not set if min and others are set to 0
;-
function HIST2D_mic, V1, V2, Weight, BINSIZE1=Binsize1, BINSIZE2=Binsize2, $
...
;  if not keyword_set( BINSIZE2 ) then Binsize2 = 1.0
  if ~(N_ELEMENTS(MAX1) GT 0)  then Max1 = m1
  if ~(N_ELEMENTS(MAX2) GT 0)  then Max2 = m2
  if ~(N_ELEMENTS(MIN1) GT 0)  then Min1 = mm1
  if ~(N_ELEMENTS(MIN2) GT 0)  then Min2 = mm2
  if not keyword_set( BINSIZE1 ) then Binsize1 = 1.0
  if not keyword_set( BINSIZE2 ) then Binsize2 = 1.0
...
;    V2c  = V2
;  endelse
  ;this was wrong and removing those inside
  iout = WHERE( (V1 gt Max1) or (V1 lt Min1) or $
