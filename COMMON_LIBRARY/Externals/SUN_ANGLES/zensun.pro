pro zensun,day,time,lat,lon,zenith,azimuth,solfac,sunrise,sunset,local=local,$
  latsun=latsun,lonsun=lonsun,rsun=rsun
  ;+
  ;
  ;  NOTE:   THIS ROUTINE IS NOT PROPERLY VECTORIZED. DO NOT USE IN VECTOR FORMAT
  ;          RB 5/26/2012
  ;
  ;
  ; ROUTINE:      zensun
  ...
  if n_elements(day) ne 1 THEN BEGIN
    print,'zensun.pro appears to be not vectorized properly.... '
    print,'do not use with vector input'
    stop
  ENDIF
