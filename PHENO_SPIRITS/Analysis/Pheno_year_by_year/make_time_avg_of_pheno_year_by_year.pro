PRO make_time_avg_of_pheno_year_by_year
  ...
  FOR i = 0, N_ELEMENTS(fns_adj_valid1_108)-1 DO BEGIN
    PRINT, time_stats(FILE_BASENAME(fns_adj_valid1_108[i])+'.bil', dir, dir+'\TIME_AVG', FILE_BASENAME(fns_adj_valid1_108[i]), [1,108])
  ENDFOR
  FOR i = 0, N_ELEMENTS(fns_adj_valid1_36)-1 DO BEGIN
    PRINT, time_stats(FILE_BASENAME(fns_adj_valid1_36[i])+'.bil', dir, dir+'\TIME_AVG', FILE_BASENAME(fns_adj_valid1_36[i]), [1,36])
  ENDFOR
  FOR i = 0, N_ELEMENTS(fns_adj_valid0_250)-1 DO BEGIN
    PRINT, time_stats(FILE_BASENAME(fns_adj_valid0_250[i])+'.bil', dir, dir+'\TIME_AVG', FILE_BASENAME(fns_adj_valid0_250[i]), [0,250], YSCALING = [0.2, 0.0048])
  ENDFOR
