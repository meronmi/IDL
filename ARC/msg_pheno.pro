PRO MSG_Pheno
lta_dir = 'D:\ARC\MSG\LTAfiles'
base_dir = 'D:\ARC\MSG'
;for run moreEOS
collection_dir = 'D:\ARC\MSG\MORE_eos'
...
FOR s = 0, N_ELEMENTS(sos_thresholds)-1 DO BEGIN
  FOR e = 0, N_ELEMENTS(eos_thresholds)-1 DO BEGIN
    ...
    all_in_a_STRUCTURE = CREATE_STRUCT($
      ...
      'prefix', 'AF_LTA', $ ;the prefix is for lta
      'suffix', '""', $
      'dateformat', '5', $
      'dir_lta', lta_dir, $ 
      ...
      'target_area_fn', base_dir + '\MSG_all_zeros.img', $
      'cm_fn', base_dir + '\MSG_all_zeros.img', $
      'devDir', 'C:\dev' $
      )
