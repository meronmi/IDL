...
;    h1 = plot(JD, inv.lai[ind], LINESTYLE='', SYMBOL='x', COLOR='r', OVERPLOT=1)
  ...
  FOR i = 0, count-1 DO BEGIN
    ...
    IF (FINITE(lai) EQ 1) THEN BEGIN
      ...
      ENDIF ELSE BEGIN 
        ...
        IDL_PRO4SAIL2_lidf, Cab,Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAsat,RAAsat,rsoil,resh,resv,absh,absa
      ENDELSE
      ...
      ENDIF ELSE BEGIN
        ...
        IDL_PRO4SAIL2_lidf, Cab,Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAcam,RAAcam,rsoil,resh,resv,absh,absa
      ENDELSE
      b_band = TOTAL(resv[subB] * sr.bSRnorm[subB])
      g_band = TOTAL(resv[subG] * sr.gSRnorm[subG])
      r_band = TOTAL(resv[subR] * sr.rSRnorm[subR])
  ...
  IF (fix_var_but_lai EQ 1) THEN suffix = 'meanVarExceptLAI_smoothed_sail2layerDifferentLidf' ELSE  suffix = ''
