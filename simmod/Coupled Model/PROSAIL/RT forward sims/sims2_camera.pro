...
lidf_a = -0.35; -1      
lidf_b = -0.15; 0  sperical corresponding to a ala = 60      
hspot  = 0.01; 0.05d      ; hot spot for grass (Atzberger et al., as above)
;#################### Overpass charcteristics
VZAsat = 0
VAAsat = 0
RAAsat = 0
VZAcam = 70; 82 ;looking down 8 deg or 8+12.5 about 20
...
;    h1 = plot(JD, inv.lai[ind], LINESTYLE='', SYMBOL='x', COLOR='r', OVERPLOT=1)
  ...
  FOR i = 0, count-1 DO BEGIN
    ...
    IF (FINITE(lai) EQ 1) THEN BEGIN
      ...
      PRINT, lai,  g_band/(b_band+g_band+r_band), (nir_band - r_band)/(nir_band + r_band), SZA
      ;camera settings
  ...
  IF (fix_var_but_lai EQ 1) THEN suffix = 'meanVarExceptLAI_smoothed' ELSE  suffix = ''
