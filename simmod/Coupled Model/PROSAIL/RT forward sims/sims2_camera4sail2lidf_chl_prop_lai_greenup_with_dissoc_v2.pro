PRO simS2_Camera4SAIL2LIDF_chl_prop_lai_greenup_with_dissoc_v2
...
;tmp = READ_CSV('X:\works\pubblicazioni\in preparazione\2017 Anton pheno s2\DATA4RT\All relevant data_lai_from_NDVI.csv', $
...
thl = 30 ;30 half life for clorophyll exp decay after max
fract_max_chl_top = 1.0 ;fraction of reference chl to be used for the top layer
rsoil_fn = 'X:\works\pubblicazioni\in preparazione\2017 Anton pheno s2\DATA4RT\ASD_soil.csv'
...
lidf_aDOWN =  -0.35; -1
lidf_bDOWN =  -0.15; 0  sperical corresponding to a ala = 60
lidf_a = -1.0; -0.35; -1      
lidf_b = 0.0;-0.15; 0 erectophile
hspot  = 0.1; 0.05d      ; hot spot for grass (Atzberger et al., as above)
fb = 4/5.0; 1.0
diss = 1.0
Cv = 1.0
zeta = 0.0
...
;#################### RUN on all cameras
envi
IF (fix_var_but_lai EQ 1) THEN suffix = 'sail2layerDifferentLidf_meanRTvar_LAI_smoothed_cabPropoLaiAndExpDec' ELSE  suffix = ''
OPENW, lunRep, outdir + '\Report_' + suffix + '.csv', /GET_LUN
dlmtr = ','
line = ['FID','JD','DD','MM','YYYY','LAI','Chl','NDVI_sat','GCC_sat','NDVI_cam','GCC_cam_S2SpecResp','GCC_cam_NikonSpecResp']
PRINTF, lunRep, STRJOIN(STRTRIM(line,2), dlmtr)
;find unique fids
...
;    h1 = plot(JD, inv.lai[ind], LINESTYLE='', SYMBOL='x', COLOR='r', OVERPLOT=1)
  ...
  FOR i = 0, count-1 DO BEGIN
    ...
    IF (fix_var_but_lai EQ 1) THEN BEGIN
      ...
      CabTopAfterMax = (Cab *fract_max_chl_top) * EXP(-(JD[sub_lai_max:-1]-JD[sub_lai_max])/thl); (Cab * 0.5)/1.0 * EXP(-(JD[sub_lai_max:-1]-JD[sub_lai_max])/thl)
      ;make chl in upper layer proportional to lai
      ;scale Chl between 0 and mean over lai min - lai max
      CabPropLai =  (lai_smooth - MIN(lai_smooth, /NAN)) / FLOAT(MAX(lai_smooth, /NAN) - MIN(lai_smooth, /NAN)) * (Cab * fract_max_chl_top)
      ;fbPropLai = (lai_smooth - MIN(lai_smooth, /NAN)) / FLOAT(MAX(lai_smooth, /NAN) - MIN(lai_smooth, /NAN)) * 0.8 + 0.2
      fbPropLai = (lai_smooth - MIN(lai_smooth, /NAN)) / FLOAT(MAX(lai_smooth, /NAN) - MIN(lai_smooth, /NAN)) ;* 0.8 + 0.2
      CabPropLai = CabPropLai[i]
      fbPropLai = fbPropLai[i]
      DinvPropLai = 1.0 - fbPropLai
      ;test for anton       CabTopAfterMax = CabTopAfterMax *0.0       CabPropLai = CabPropLai * 0.0
    ENDIF ELSE BEGIN
      Cab = inv.chl[ind[i]]
      Cm = inv.cm[ind[i]]
      Cw = inv.cw[ind[i]]
    ...
    IF (FINITE(lai) EQ 1) THEN BEGIN
      ...
      IF (i GT sub_lai_max) THEN BEGIN
        CabTs = [CabTs, CabTopAfterMax[i-sub_lai_max]]
        fbTs = [fbTs, fb]
        ...
        IDL_PRO4SAIL2_lidf, CabTopAfterMax[i-sub_lai_max],Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAsat,RAAsat,rsoil,resh,resv,absh,absa 
      ENDIF ELSE BEGIN 
        ;before max all equal
        CabTs = [CabTs, 0.0]
        fbTs = [fbTs, fbPropLai]
        ;IDL_PRO4SAIL2_lidf, CabPropLai,Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAsat,RAAsat,rsoil,resh,resv,absh,absa
        ;IDL_PRO4SAIL2_lidf, CabPropLai,Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAsat,RAAsat,rsoil,resh,resv,absh,absa
        
        IDL_PRO4SAIL2_lidf, 0.0,Car,Cbrown,0.0,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_aDOWN,lidf_bDOWN,lidf_aDOWN,lidf_bDOWN,lai,hspot,fbPropLai,DinvPropLai,Cv,zeta, SZA,VZAsat,RAAsat,rsoil,resh,resv,absh,absa
      ENDELSE
      ...
      IF (i GT sub_lai_max) THEN BEGIN
        ...
        IDL_PRO4SAIL2_lidf, CabTopAfterMax[i-sub_lai_max],Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAcam,RAAcam,rsoil,resh,resv,absh,absa
      ...
      ENDIF ELSE BEGIN
       ...
       ; IDL_PRO4SAIL2_lidf, CabPropLai,Car,Cbrown,Cw,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_a,lidf_b,lidf_aDOWN,lidf_bDOWN,lai,hspot,fb,diss,Cv,zeta, SZA,VZAcam,RAAcam,rsoil,resh,resv,absh,absa
        IDL_PRO4SAIL2_lidf, 0.0,Car,Cbrown,0.0,Cm,Ns,Cab,Car,Cbrown,Cw,Cm,Ns,lidf_aDOWN,lidf_bDOWN,lidf_aDOWN,lidf_bDOWN,lai,hspot,fbPropLai,DinvPropLai,Cv,zeta, SZA,VZAcam,RAAcam,rsoil,resh,resv,absh,absa
      ENDELSE
      ;PRINT, lai
      b_band = TOTAL(resv[subB] * sr.bSRnorm[subB])
      g_band = TOTAL(resv[subG] * sr.gSRnorm[subG])
    ...
    ENDELSE
    line = [uniqueFids[f],JD[i],DD,MM,YY,lai,CabTs[-1],ndvi_sat[-1],GCC_sat[-1],ndvi_cam[-1],GCC_cam[-1], GCC_cam_camSR[-1]]
    PRINTF, lunRep, STRJOIN(STRTRIM(line,2), dlmtr)
  ENDFOR
  ...
  ghs0 = PLOT(JD, ndvi_sat, YTITLE='NDVI, LAIx0.1, Chlx0.02', COLOR = 'black', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XRANGE=xrange, YRANGE=yrange, $ 
            TITLE = 'Fid '+ STRTRIM(uniqueFids[f],2) + ' Chl half life=' + STRTRIM(thl,2), XMINOR=xminor, XMAJOR = nmajor, $
  ...
  ghCab = PLOT(JD, cabTs/50.0,  NAME='Chlx0.02', COLOR='RED', OVERPLOT = 1, LINESTYLE='-', SYMBOL ='D')
  ghFb = PLOT(JD, fbTs,  NAME='FractBotLayer', COLOR='black', OVERPLOT = 1, LINESTYLE='-')
  ...
  ghc3 = PLOT(JD, GCC_cam_camSR, NAME='GCC cam camSR', LINESTYLE='-', SYMBOL ='o', COLOR='YELLOW GREEN', /CURRENT, YRANGE = gccrange, MARGIN = margin, AXIS_STYLE = 0, XRANGE=xrange)
  ...
  !null = LEGEND(target=[ghs1, ghs2, ghc2, ghc3, ghlai, ghCab, ghFb], /AUTO_TEXT_COLOR, SHADOW=0, LINESTYLE=6, SAMPLE_WIDTH=0.1, TRANSPARENCY=100, FONT_SIZE=10)
