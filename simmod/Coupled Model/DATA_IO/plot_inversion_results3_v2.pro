FUNCTION plot_inversion_results3_v2, buffer_site_year_plot, doPlotSpectra, $
  satR1, satR2, satR3, satR4, satR5, satR6, satR7, satObsJD, $
  tJDo, indJD0_ecmwf, ec_JDo, INDJD0_eddy, $
  ecmwf_globrado, ec_globrado, ecmwf_tairo, ec_tairo, $
  ecmwf_preco, ec_preco, $
  simo, ec_GDDo, ecmwf_GDDo, ec_gpp_mdso, ret, sYear, prosailvar, $
  useEddyMet, tJD_of_ratio_bewm_LTAbewm, ratio_bewm_LTAbewmo, out_dir, base_name, $
  ...
  ratio_bewm_LTAbewmo = ratio_bewm_LTAbewmo[ind:ind+N_ELEMENTS(tJDo)-1]
  ...
  IF (resample) THEN BEGIN
    tmp=0
    resample2MODIS8days, tJDo*0, tJDo, tmp, tJD
    n = N_ELEMENTS(tJD)
    sim = { $
      ...
      tJD:            FLTARR(n)*0.0, $  ;here store the resampled tJD
      NRTtJDOfMax:    0.0}


    resample2MODIS8days, ecmwf_globrado, tJDo, ecmwf_globrad, tJD
    resample2MODIS8days, ecmwf_preco, tJDo, ecmwf_prec, tJD
    resample2MODIS8days, ecmwf_gddo, tJDo, ecmwf_gdd, tJD
    resample2MODIS8days, ratio_bewm_LTAbewmo, tJDo, ratio_bewm_LTAbewm, tJD
    resample2MODIS8days, ecmwf_tairo, tJDo, ecmwf_tair, tJD


    resample2MODIS8days, simo.fa, tJDo, tmp, tJD & sim.fa = tmp
    sim.tJD = tJD
    resample2MODIS8days, simo.lai, tJDo, tmp, tJD & sim.lai = tmp
    resample2MODIS8days, simo.dlai, tJDo, tmp, tJD & sim.dlai = tmp
    resample2MODIS8days, simo.dlai_senescent, tJDo, tmp, tJD & sim.dlai_senescent = tmp
    resample2MODIS8days, simo.gpp, tJDo, tmp, tJD & sim.gpp = tmp
    resample2MODIS8days, simo.P1, tJDo, tmp, tJD & sim.P1 = tmp
    resample2MODIS8days, simo.eps_s, tJDo, tmp, tJD & sim.eps_s = tmp
    ...
    ;eddy data are resampled by themselves as it is not certain that all data are there

    resample2MODIS8days, ec_globrado, ec_JDo, ec_globrad, ec_JD
    resample2MODIS8days, ec_preco, ec_JDo, ec_prec, ec_JD
    resample2MODIS8days, ec_tairo, ec_JDo, ec_tair, ec_JD
    resample2MODIS8days, ec_GDDo, ec_JDo, ec_GDD, ec_JD
    resample2MODIS8days, ec_gpp_mdso, ec_JDo, ec_gpp_mds, ec_JD
  ENDIF ELSE BEGIN
    sim = simo
    ecmwf_globrad = ecmwf_globrado
    ecmwf_prec = ecmwf_preco
    ecmwf_gdd = ecmwf_gddo
    ratio_bewm_LTAbewm = ratio_bewm_LTAbewmo
    ecmwf_tair = ecmwf_tairo

    ec_globrad = ec_globrado
    ec_prec = ec_preco
    ec_tair = ec_tairo
    ec_GDD = ec_GDDo
    ec_gpp_mds = ec_gpp_mdso
  ENDELSE
  ...
  ;Radiation, ECMWF
  pos = pos0
  yrange =[0,MAX([ecmwf_globrad, ec_globrad], /NAN)+10.0]
  gh_radg_ecmwf = PLOT(tJD, ecmwf_globrad, YTITLE='Globrad (MJ $m^{-2}$)', COLOR = 'black', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
    FONT_SIZE = fs, POSITION=pos, DIMENSIONS=dm, XRANGE=xrange, Name='ECMWF', YRANGE=yrange, WINDOW_TITLE = base_name, BUFFER = buffer_site_year_plot)
  ...
  ;Radiation, EC flux data
  gh_radg_eddy = PLOT (ec_JD, ec_globrad, COLOR = 'red', OVERPLOT = 1, Name='EC',  YRANGE=yrange)
  ...
  !null = LEGEND(target=[gh_radg_ecmwf, gh_radg_eddy], /AUTO_TEXT_COLOR, FONT_SIZE = fs-1, POSITION=lp, /RELATIVE, $
    SHADOW=0, LINESTYLE=6, SAMPLE_WIDTH=0.05, TRANSPARENCY=100, HORIZONTAL_SPACING=0.02, VERTICAL_SPACING = 0.015) 
  ...
  ;Precipitation, EC flux data
  gh_rain_eddy = PLOT (ec_JD, ec_prec, COLOR = 'red', OVERPLOT = 1, Name='EC', YRANGE=yrange)
  gh_tmp = PLOT([satObsJD[ind_max_ndvi_obs], satObsJD[ind_max_ndvi_obs]], yrange, COLOR='grey', OVERPLOT=1);, YRANGE=yrange)
  ...
  ;Temperature, EC flux data
  ;gh_gdd_eddy = PLOT (ec_JD[indJD0_eddy:*], ec_GDD, COLOR = 'red', OVERPLOT = 1, Name='GDD EC',  AXIS_STYLE = axSt)
  gh_gdd_eddy = PLOT (ec_JD[indJD0_eddy:*], ec_GDD, COLOR = 'red', OVERPLOT = 1, Name='GDD EC',  AXIS_STYLE = axSt)
  ...
  gh_t_eddy  = PLOT(ec_JD[indJD0_eddy:*], ec_tair,  COLOR = 'red', NAME = 'T EC', AXIS_STYLE = 0, $
    XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', LINESTYLE=':', $
    FONT_SIZE = fs, POSITION=pos, YRANGE = yrange, XRANGE=xrange, /CURRENT)
  ...
  lp = [0.5, 1]
  null = LEGEND(target=[gh_gdd_ecmwf, gh_gdd_eddy, gh_t_ecmwf, gh_t_eddy], /AUTO_TEXT_COLOR, FONT_SIZE = fs-3, POSITION=lp, /RELATIVE, $
    SHADOW=0, LINESTYLE=6, SAMPLE_WIDTH=0.05, TRANSPARENCY=100, HORIZONTAL_SPACING=0.02, VERTICAL_SPACING = 0.015)
  lp = [1.05, 1]
  ;LUE
  pos = pos0
  ...
  ;Note: eddy data and simulated data may not share the same time domain (ec_JD and tJD), adjust for this
  match, ec_JD, tJD, sub_ec, sub_t  ;sub_ec and sub_t are the sunscripts to the common elemnts
  common_act_Eps_JD = ec_JD[sub_ec]
  common_ec_globrad = ec_globrad[sub_ec]
  common_ec_gpp = ec_gpp_mds[sub_ec]
  common_sim_fa = sim.fa[sub_t]
  ;sim_gpp = sim.gpp[sub_t]
  ;apar = ec_globrad*globrad2par*sim.fa
  apar = common_ec_globrad*globrad2par*common_sim_fa
  ...
  actEps_s_sim_fAPAR =  common_ec_gpp/apar/eps_max
  gh_actLUE_ecmwf = PLOT(common_act_Eps_JD, actEps_s_sim_fAPAR,  COLOR = 'red', $;AXIS_STYLE = 0, $
    XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
    FONT_SIZE = fs, POSITION=pos, /CURRENT, XRANGE=xrange, YRANGE=yrange, Name='actEps_s_sim_fAPAR', YTITLE='$\epsilon_w_s$, EWP:EWPET (-)', /NODATA)
  loc = -(yrange[1]-yrange[0])/10
  strlabel = STRTRIM(ROUND(jd2doy([xrange[0], xrange[0]+(xrange[1]-xrange[0])/(nmajor-1), xrange[0]+2*(xrange[1]-xrange[0])/(nmajor-1), xrange[1]])),2)
  ...
  ;ratio
  gh_ratio = PLOT (tJD, ratio_bewm_LTAbewm,  COLOR = 'violet', OVERPLOT=1, Name='EWP:EWPET')
   lp = [1, 1]
  !null = LEGEND(target=[gh_bewm_ecmwf, gh_ratio], /AUTO_TEXT_COLOR, FONT_SIZE = fs-1, POSITION=lp, /RELATIVE, $
    SHADOW=0, LINESTYLE=6, SAMPLE_WIDTH=0.05, TRANSPARENCY=100, HORIZONTAL_SPACING=0.02, VERTICAL_SPACING = 0.015)
  ...
  a_lai = AXIS('y', TARGET = gh_lai, LOCATION = [max(gh_lai.xrange),0,0], TICKFONT_SIZE = fs, TEXTPOS = 1, $
    TITLE = 'LAI ($m^2$$m^{-2}$)', YRANGE=yrange, TICKDIR=1)
  ...
  ;Note: eddy data and simulated data may not share the same time domain (ec_JD and tJD), adjust for this
  match, ec_JD, tJD, sub_ec, sub_t  ;sub_ec and sub_t are the sunscripts to the common elemnts
  common_JD = ec_JD[sub_ec]
  ec_gpp = ec_gpp_mds[sub_ec]
  sim_gpp = sim.gpp[sub_t]
  ;set negative gpp to NaN
  indfin = WHERE(FINITE(ec_gpp))
  indeg = WHERE(ec_gpp[indfin] LT 0.0, nneg)
  IF (nneg GT 0) THEN ec_gpp[indfin[indeg]] = !VALUES.F_NAN
  ;indfin = WHERE(FINITE(ec_gpp_mds))
  ;find where i smax NDVI
  indmax = WHERE(common_JD GE satObsJD[ind_max_ndvi_obs])
  indmax = indmax[0]
  ;eddy data may be incomplete, e.g. no data after a give time
  yrange=[-0.5, MAX([ec_gpp, sim_gpp], /NAN)+0.2]
  ;adjust case of eddy data ending before the max
  gh_scatterA = !NULL
  gh_scatterB = !NULL
  IF (indmax EQ -1) THEN BEGIN
    h_scatterB = PLOT(ec_gpp, sim_gpp, SYMBOL='+', LINESTYLE='none', XRANGE=yrange, YRANGE=yrange, $
      FONT_SIZE = fs, POSITION=pos, /CURRENT, NAME = 't<=t_max_ndvi', XTITLE='EC GPP (>=0)', YTITLE='sim GPP')
  ENDIF ELSE BEGIN
    ;plot black until the ndvi max
    IF (TOTAL(FINITE(ec_gpp[0:indmax])) GT 0) THEN $
      gh_scatterB = PLOT(ec_gpp[0:indmax], sim_gpp[0:indmax], SYMBOL='+', LINESTYLE='none', XRANGE=yrange, YRANGE=yrange, $
        FONT_SIZE = fs, POSITION=pos, /CURRENT, NAME = 't<=t_max_ndvi', XTITLE='EC GPP (>=0)', YTITLE='sim GPP')
    ;adust the case where ther are no eddy data after
    IF (indmax LT N_ELEMENTS(ec_gpp)-1) THEN BEGIN
      IF (TOTAL(FINITE(ec_gpp[indmax+1:*])) GT 0) THEN BEGIN
        IF (TOTAL(FINITE(ec_gpp[0:indmax])) GT 0) THEN $
      ;and grey afterwards sim.gpp[0:N_ELEMENTS(ec_gpp_mds)-1]
          gh_scatterA = PLOT(ec_gpp[indmax+1:*], sim_gpp[indmax+1:*], SYMBOL='+', LINESTYLE='none', $
            FONT_SIZE = fs, COLOR='grey', OVERPLOT=1, XRANGE=yrange, YRANGE=yrange, NAME = 't>t_max_ndvi') ELSE $
            gh_scatterA = PLOT(ec_gpp[indmax+1:*], sim_gpp[indmax+1:*], SYMBOL='+', LINESTYLE='none', $
              FONT_SIZE = fs, COLOR='grey', POSITION=pos, /CURRENT, XRANGE=yrange, YRANGE=yrange, NAME = 't>t_max_ndvi')
      ENDIF
    ENDIF
  ENDELSE
  ...
  GPP_simVSobs = {R2:!VALUES.F_NAN, slope:!VALUES.F_NAN, offset:!VALUES.F_NAN, percGPPdelta:!VALUES.F_NAN, rmse:!VALUES.F_NAN, bias:!VALUES.F_NAN}
  GPP_modisVSobs = {R2:!VALUES.F_NAN, slope:!VALUES.F_NAN, offset:!VALUES.F_NAN, percGPPdelta:!VALUES.F_NAN, rmse:!VALUES.F_NAN, bias:!VALUES.F_NAN}
  indfin = WHERE(FINITE(ec_gpp), countfin)
  IF (countfin GT 3) THEN BEGIN
    b = REGRESS(ec_gpp[indfin], sim_gpp[indfin], CONST = a, CORRELATION = r)
    ...
    GPP_simVSobs.rmse = SQRT(TOTAL((sim_gpp[indfin]-ec_gpp[indfin])^2) / FLOAT(countfin))
    GPP_simVSobs.bias = TOTAL(sim_gpp[indfin]-ec_gpp[indfin]) / FLOAT(countfin)
    ...
    IF (RefGppAvailable) THEN BEGIN
      ...
      modisGpp = extractModisVal(RefGpp_data.JD, RefGpp_data.Gpp_daily, common_JD)
      b = REGRESS(ec_gpp[indfin], modisGpp[indfin], CONST = a, CORRELATION = r)
      ...
      GPP_modisVSobs.rmse = SQRT(TOTAL((modisGpp[indfin]-ec_gpp[indfin])^2) / FLOAT(countfin))
      GPP_modisVSobs.bias = TOTAL(modisGpp[indfin]-ec_gpp[indfin]) / FLOAT(countfin)
    ENDIF
  ENDIF 
  ...
  yrange = [0.0,1.0]
  indf = WHERE(FINITE(ndvi_sim))
  gh_sim_ndvi = PLOT (tJDo[indf], ndvi_sim[indf], YTITLE='MODIS NDVI',  COLOR = 'black', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
    FONT_SIZE = fs, POSITION=pos, /CURRENT, NAME = 'simNDVI', YRANGE=yrange, XRANGE=xrange, $
    DIMENSIONS=dm, WINDOW_TITLE = base_name + ' MODEL');, LINESTYLE='none', SYMBOL='+', SYM_SIZE = 0.7)
  loc = -(yrange[1]-yrange[0])/10
  strlabel = STRTRIM(ROUND(jd2doy([xrange[0], xrange[0]+(xrange[1]-xrange[0])/(nmajor-1), xrange[0]+2*(xrange[1]-xrange[0])/(nmajor-1), xrange[1]])),2)
  ...
  str_format =  '("____",  A-14, "_",         A-7,"__",  A-7, "__",  A-7,"_",           A-5,"__",    A-7, "___",     A-5, "__",  A-8,"___",   A-7,"___",   A-4, "__",  A-6, "__",   A-7,"__", A-7)'
  par_format_fg='("fg:__", I03,"__", F9.1,"__", F6.4,"__", F6.4,"___", F5.3,"____",     F4.2,"____",  F07.2,"___",   F5.3,"__", F07.2,"___",   F7.4,"___",  F6.2,"__",  F05.1,"___", F06.1,"___", F07.2)'
  par_format_p ='("p :__", I03,"__", F9.1,"__", F6.4,"__", F6.4,"___", F5.3,"____",     F4.2,"____",  F07.2,"___",   F5.3,"__", F07.2,"___",   F7.4,"___",  F6.2,"__",  F05.1,"___", F06.1,"___", F07.2)'
  ...
  IF (ret.info_mpfit.npegged GT 0) THEN BEGIN
    ...
    ind_par_pegged_ll = WHERE((ret.info_mpfit.parinfo[*].FIXED EQ 0) AND ((ret.info_mpfit.parms - ret.info_mpfit.parinfo[*].LIMITS[0]) EQ 0.0), count_par_pegged_ll)
    IF (count_par_pegged_ll GT 0) THEN  tmp[ind_par_pegged_ll] = tmp[ind_par_pegged_ll] + '*L'
    ind_par_pegged_ul = WHERE((ret.info_mpfit.parinfo[*].FIXED EQ 0) AND ((ret.info_mpfit.parms - ret.info_mpfit.parinfo[*].LIMITS[1])) EQ 0.0, count_par_pegged_ul)
  ...
  th0 = TEXT(0.01, 0.98, tmp + 'Year: '+ STRTRIM(sYear,2)+ '; '+ line0, FONT_SIZE = 10, FONT_NAME = "Helvetica")
  th1 = TEXT(0.01, 0.96,line1,FONT_NAME = "Helvetica", FONT_SIZE = 10)
  th2 = TEXT(0.01, 0.94, line2,FONT_NAME = "Helvetica", FONT_SIZE = 10)
  th3 = TEXT(0.01, 0.92, line3,FONT_NAME = "Helvetica", FONT_SIZE = 10)
  ...
  gh_sim_ndvi = PLOT (tJDo[indf], ndvi_sim[indf], YTITLE='MODIS NDVI',  COLOR = 'black', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
    ...
    DIMENSIONS=dm, WINDOW_TITLE = base_name + ' SPECTRAL', BUFFER = buffer_site_year_plot);, LINESTYLE='none', SYMBOL='+', SYM_SIZE = 0.7)
  loc = -(yrange[1]-yrange[0])/10
  strlabel = STRTRIM(ROUND(jd2doy([xrange[0], xrange[0]+(xrange[1]-xrange[0])/(nmajor-1), xrange[0]+2*(xrange[1]-xrange[0])/(nmajor-1), xrange[1]])),2)
  a_x =AXIS('X', TARGET = gh_sim_ndvi, LOCATION=loc, MAJOR = nmajor, MINOR=xminor, COORD_TRANSFORM=[-tJD[0]+JD2DOY(tJD[0]), 1], TICKFONT_SIZE = fs, TITLE='Time', TICKLEN=ticlensec, TICKNAME=strlabel)
  ;Observed MODIS NDVI
  ...
  indf = WHERE(FINITE(siswi_sim))
  gh_sim_siwsi = PLOT (tJDo[indf], siswi_sim[indf], COLOR = 'blue', OVERPLOT=1, $
    NAME = 'sim-SIWSI')
  gh_tmp = PLOT([satObsJD[ind_max_ndvi_obs], satObsJD[ind_max_ndvi_obs]], COLOR='grey', [yrange[0],yrange[1]], OVERPLOT=1)
  ;NDVWI
  ...
  indf = WHERE(FINITE(ndwi_sim))
  gh_sim_ndwi = PLOT (tJDo[indf], ndwi_sim[indf], COLOR = 'green', OVERPLOT=1, $
    NAME = 'sim-NDWI')
  ...
  indf = WHERE(FINITE(sim.ModRef[0,*]))
  gh_sim1 = PLOT(tJDo[indf], sim.ModRef[0,indf], COLOR = 'red', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
  ...
  ;scatter with LAI
  pos[[0,2]]= pos[[0,2]] + 0.33
  indF = WHERE(FINITE(siswi_obs), countF)
  IF (countF GT 4) THEN BEGIN
  ...
  ;plot black until the ndvi max
  ;adjust case of  max being the last point
    gh_scatterA = !NULL
    gh_scatterB = !NULL
    ...
    ;and grey afterwards
    IF (indmax LT N_ELEMENTS(tmpLai)-1) THEN BEGIN
    ...
    ENDIF
    both_finite = WHERE(FINITE(tmpLai) AND FINITE(siswi_obs), count_both_finite)
    IF (count_both_finite GE 4) THEN BEGIN
  ...
  IF (countF GE 4) THEN BEGIN
    ...
    tJDresidual = tJD[WHERE(FINITE(siswi_obs))]
    ...
    ;first align the vectors (resilduals and actEps_s_sim_fAPAR)
    match, tJDresidual, common_act_Eps_JD, sud_residuals, sub_actEps
    resiActEps_JD = tJDresidual[sud_residuals]
    com_residuals = residuals[sud_residuals]
    com_actEps = actEps_s_sim_fAPAR[sub_actEps]
    pos[[0,2]]= pos[[0,2]] + 0.33
    yrange = [MIN(com_actEps, /NAN),MAX(com_actEps, /NAN)] & ofst = (yrange[1]-yrange[0])/10.0 & yrange = [yrange[0]-ofst,yrange[1]+ofst]
    ;IF (yrange[1] GT 5) THEN yrange[1] = 5 & IF (yrange[0] LT -5) THEN yrange[0] = -5
    xrange = [MIN(com_residuals, /NAN),MAX(com_residuals, /NAN)] & ofst = (xrange[1]-xrange[0])/10.0 & xrange = [xrange[0]-ofst,xrange[1]+ofst]
    IF (yrange[1] GT ylim) THEN yrange[1] = ylim & IF (yrange[0] LT -ylim) THEN yrange[0] = -ylim
    ;plot black until the ndvi max
    indmax = WHERE(resiActEps_JD GE satObsJD[ind_max_ndvi_obs])
    indmax = indmax[0]
    ;adjust case of eddy data ending before the max
    gh_scatterA = !NULL
    gh_scatterB = !NULL
    IF (indmax EQ -1) THEN BEGIN
      gh_scatterB = PLOT(com_residuals, com_actEps, SYMBOL='+', LINESTYLE='none', XRANGE=xrange, YRANGE=yrange, $
        FONT_SIZE = fs, POSITION=pos, /CURRENT, NAME = 't<=t_max_ndvi', XTITLE='res(SISWIvsNDVI)', YTITLE='actEps_s_sim_fAPAR<'+STRTRIM(ylim,2))
    ENDIF ELSE BEGIN
      gh_scatterB = PLOT(com_residuals[0:indmax], com_actEps[0:indmax], SYMBOL='+', LINESTYLE='none', XRANGE=xrange, YRANGE=yrange, $
        FONT_SIZE = fs, POSITION=pos, /CURRENT, NAME = 't<=t_max_ndvi', XTITLE='res(SISWIvsNDVI)', YTITLE='actEps_s_sim_fAPAR<'+STRTRIM(ylim,2))
      ;adust the case where ther are no eddy data after
      IF (indmax LT N_ELEMENTS(com_actEps)-1) THEN BEGIN
        ;and grey afterwards sim.gpp[0:N_ELEMENTS(ec_gpp_mds)-1]
        gh_scatterA = PLOT(com_residuals[indmax+1:*], com_actEps[indmax+1:*], SYMBOL='+', LINESTYLE='none', $
          FONT_SIZE = fs, COLOR='grey', OVERPLOT=1, XRANGE=xrange, YRANGE=yrange, NAME = 't>t_max_ndvi')
      ENDIF
    ENDELSE
  
    ; actEps_s_sim_fAPAR may contain NaN
    ;this is done to avoid having a illegl oper error on com_actEps LT ylim
    ...
    indFinLtY = WHERE((FINITE(com_residuals)) AND (FINITE(com_actEps)) AND (goodAct))
    IF (N_ELEMENTS(indFinLtY) GT 3) THEN BEGIN
      b = REGRESS(com_residuals[indFinLtY], com_actEps[indFinLtY], CONST = a, CORRELATION = r, YFIT=yfit)
      ;trick to avoid plotting outside
      x0=(yrange[0]-a)/b[0] & x1=(yrange[1]-a)/b[0]
      IF (x0 LT xrange[0]) THEN x0=xrange[0] & IF (x1 GT xrange[1]) THEN x1=xrange[1]
      gh_reg = PLOT([x0,x1],  a+b[0]*[x0,x1], OVERPLOT=1, COLOR='blue', NAME='Reg, R2='+STRTRIM(r^2,2), YRANGE=yrange)
