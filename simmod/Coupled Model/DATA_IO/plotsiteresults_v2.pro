FUNCTION plotSiteResults_v2, lombratio, siteEc_JD, siteSimNDVIJD, NDVIobsJD, siteEc_gpp, siteResmplSimGpp_offSeason, siteModisGpp, siteModisNDVI,  siteSimNDVI, igbp, code, site_run_reportFn, out_dir

  fs = 11 ;font size
  symsz = 0.8
  lp = [0.5, 1.15] ;legend position
  dm = [800,400]  ;dimensions of the window
...
;  pos = [xmin, ymin, xmin + graph_dim*3.75, ymin+graph_dim] ;0.24 in width and height
  ...
  yoff= 0.025
  pos = [0.075, 0.5+yoff, 0.7,0.9+yoff] ;[xmin, ymin, xmax, ymax]
  gh_ec = PLOT(siteEc_JD, siteEc_gpp, YTITLE='Daily GPP (gC $m^{-2}$$d^{-1}$)', XTITLE='Time', COLOR = 'red', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
    FONT_SIZE = fs, POSITION=pos, DIMENSIONS=dm, XSHOWTEXT = 0,$
    XRANGE=xrange, Name='EC', YRANGE=yrange, WINDOW_TITLE = igbp + '_' + code)
  ...
  gh_nd = PLOT(NDVIobsJD, siteModisNDVI, COLOR = 'red',  /CURRENT, Name='MODIS', LINESTYLE='none', SYMBOL='+', /NODATA, AXIS_STYLE=0)
  indf = WHERE(FINITE(siteSimNDVI))
  gh_sim_nd = PLOT (siteSimNDVIJD[indf], siteSimNDVI[indf], COLOR = 'black', OVERPLOT=1, Name='Sim', LINESTYLE='none', SYMBOL='o', SYM_SIZE=0.5, /NODATA, AXIS_STYLE=0)
  !null = LEGEND(target=[gh_ec, gh_sim, gh_Modis, gh_nd, gh_sim_nd], /AUTO_TEXT_COLOR, FONT_SIZE = fs, POSITION=lp, HORIZONTAL_ALIGNMENT = 'CENTER', ORIENTATION = 1, /RELATIVE, $
    SHADOW=0, LINESTYLE=6, SAMPLE_WIDTH=0.1, TRANSPARENCY=100)

  ;NDVI
  pos = [0.075, 0.1+yoff, 0.7,0.5+yoff]
  ...
  gh_nd = PLOT(NDVIobsJD, siteModisNDVI, YTITLE='NDVI (-)',  XTITLE='Time',COLOR = 'red', XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', XMINOR=xminor, $
...
;  !null = LEGEND(target=[gh_nd, gh_sim], /AUTO_TEXT_COLOR, FONT_SIZE = fs-1, POSITION=lp,  HORIZONTAL_ALIGNMENT = 'CENTER', ORIENTATION = 1,  /RELATIVE, $
;    SHADOW=0, LINESTYLE=6, SAMPLE_WIDTH=0.1, TRANSPARENCY=100)

  ;scatter plots
  ;SIM
  ;pos = [xmin, ymin-0.68, xmin + graph_dim, ymin + graph_dim-0.68]
  pos = [0.785, 0.5+yoff,0.985,0.9+yoff]
  yrange=[-3, MAX([siteEc_gpp, siteResmplSimGpp_offSeason, siteModisGpp], /NAN)+0.2]
  ...
  h_scatter = PLOT(siteEc_gpp, siteResmplSimGpp_offSeason, SYMBOL='+', LINESTYLE='none', XRANGE=yrange, YRANGE=yrange,  XSHOWTEXT = 0, $
    FONT_SIZE = fs, POSITION=pos, /CURRENT, YTITLE='Sim (gC $m^{-2}$$d^{-1}$)', SYM_SIZE=symsz)    ;XTITLE='EC GPP (gC $m^{-2}$ $d^{-1}$)'
  gh_11 = PLOT(yrange, yrange, OVERPLOT=1, COLOR='black', NAME='1:1 line', LINESTYLE='--')
  ...
  x0=(yrange[0]-a_sim)/b_sim[0];(0.0-a_sim)/b_sim[0]
  ...
  th1 = TEXT(pos[2]-0.18, pos[3]-0.05, '$R^2$='+STRING(r_sim^2, FORMAT='(F4.2)', /PRINT)+', RMSE=' + STRING(RMSE_sim, FORMAT='(F4.2)', /PRINT), COLOR='blue')
  ;Modis
  ;pos[[0,2]]= pos[[0,2]] + 0.30
  
  pos = [0.785, 0.1+yoff,0.985,0.5+yoff] 
  
  h_scatter = PLOT(siteEc_gpp, siteModisGpp, SYMBOL='+', LINESTYLE='none', XRANGE=yrange, YRANGE=yrange, color='green', $
    FONT_SIZE = fs, POSITION=pos, /CURRENT, XTITLE='EC GPP (gC $m^{-2}$$d^{-1}$)', YTITLE='MOD17 (gC $m^{-2}$ $d^{-1}$)', SYM_SIZE=symsz)
  gh_11 = PLOT(yrange, yrange, OVERPLOT=1, COLOR='black', NAME='1:1 line', LINESTYLE='--')
