FUNCTION bridge_SimMod2Phenot365ndvixglobrad, modis_data, globrad, force_mono
  @cb_job.comm
  @cb_in.comm
  @cb_stat.comm
  @cb_frst.comm
  ...
  ;plot (0 to suppress)
  pltres  = 1  ;this is for pheno plot and to make the plot of original 8 day NDVI from MODIS and 10 day resampled for Pheno365
  ...
  globrad_dek = INTERPOL(globrad, modis_data.JD, jd, /NAN)
  nxg =  globrad_dek*ndvi_dek
  nxg = nxg / (max(nxg, /NAN)-min(nxg, /NAN))
  IF (pltres) THEN BEGIN 
    dummy = LABEL_DATE(DATE_FORMAT=['%D/%N','%Y'])
    h = PLOT(modis_data.JD[indFin], ndvi, SYMBOL = 1, LINESTYLE=' ', AXIS_STYLE = 1, XTICKFORMAT='(C(CDI,1x,CMoA,1x,CYI2))', DIMENSION=[1500,300])
    h = PLOT(jd, ndvi_dek, color = 'red', SYMBOL = 1, LINESTYLE=' ',OVERPLOT=1)
    h = PLOT(jd, nxg, color = 'blue', SYMBOL = 1, LINESTYLE=' ',OVERPLOT=1)
  ENDIF
  ...
  rc = phenot_funct (REFORM(nxg), REFORM(jd))
  tmp = '#################################################################'
