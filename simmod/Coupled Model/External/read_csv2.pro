;MODYFIED TO DETERMINE DATA TYPE BASED ON ALL ROWS
;MODIFY so that is not automatically converted to double only if it has the "." inside
...
function read_csv2, Filename, $
  ...
  if (count gt 1) then begin
    ...
    for j=0,nColumns-1 do begin
      ...
      ;MM
      isValidNUM = MIN(VALID_NUM(subdata)) 
      
      ;if (hasDecimal || ~ARRAY_EQUAL(tmpLong64, tmpDouble)) then begin
      if ((hasDecimal || ~ARRAY_EQUAL(tmpLong64, tmpDouble)) AND isValidNUM) then begin
      ;MM
        ; Double
        types[j] = 5
      endif else begin
